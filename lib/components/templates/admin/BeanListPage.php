...
class BeanListPage extends AdminPageTemplate
{
    ...
    protected function setBeanQuery() : void
    {
        ...
        $qry = $this->bean->query($this->bean->key());

        foreach($this->fields as $name=>$label) {
            if ($this->bean->haveColumn($name)) {
                $qry->stmt->column($name);
            }
        }
    ...
    protected function initViewActions(ActionCollection $act) : void
    {
        ...
        if ($this->bean instanceof OrderedDataBean) {
            ...
            $action_prev->setTooltip("Move element one position backward");
            $act->append($action_prev);

            $act->append(Action::PipeSeparator());

            $action_next = new Action("Next", "?cmd=reposition&type=next", $repos_param);
            $action_next->setTooltip("Move element one position forward");
