...
class PayPalProcessor {
	...
	public function getToken(PayPalSale $sale, $PayPalReturnURL, $PayPalCancelURL, $no_shipping=0)
	{
		...
		$padata = 	"&METHOD=SetExpressCheckout".
				...
				"&L_PAYMENTREQUEST_0_NAME0=".urlencode($sale->getItemName()).
				"&L_PAYMENTREQUEST_0_NUMBER0=".urlencode($sale->getItemNumber()).
				"&L_PAYMENTREQUEST_0_DESC0=".urlencode($sale->getItemDesc()).
				"&L_PAYMENTREQUEST_0_AMT0=".urlencode($sale->getItemPrice()).
				"&L_PAYMENTREQUEST_0_QTY0=". urlencode($sale->getItemQty()).

				"&NOSHIPPING={$no_shipping}". //set 1 to hide buyer"s shipping address, in-case products that does not require shipping
				
				"&PAYMENTREQUEST_0_ITEMAMT=".urlencode($sale->getTotalPrice()).
				"&PAYMENTREQUEST_0_TAXAMT=".urlencode($sale->getTaxAmount()).
				"&PAYMENTREQUEST_0_SHIPPINGAMT=".urlencode($sale->getShippingCost()).
				"&PAYMENTREQUEST_0_HANDLINGAMT=".urlencode($sale->getHandlingCost()).
				"&PAYMENTREQUEST_0_INSURANCEAMT=".urlencode($sale->getInsuranceCost()).
				"&PAYMENTREQUEST_0_AMT=".urlencode($sale->getGrandTotal()).
	...
	public function confirmPayment(PayPalSale $sale)
	{
			...
			$padata = 	"&TOKEN=".urlencode($sale->getPaymentToken()).
						"&PAYERID=".urlencode($sale->getPayerID()).
						"&PAYMENTREQUEST_0_PAYMENTACTION=".urlencode("SALE").
