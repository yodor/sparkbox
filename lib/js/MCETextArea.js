...
{
  console.log("Image Clicked. imageID: "+imageID);
  ...
  this.req.start(
	...
	confirm_button.on("click", function(event){
	    ...
	    if (popup.is(":checked")) {
		var popup_href = SITE_ROOT+"storage.php?cmd=gallery_photo&class=MCEImagesBean&id="+imageID;
...
MCEImageBrowserDialog.prototype.show = function(textarea)
{
    ...
    var field = modal_pane.popup().find(".UploadControl").first();
...
MCETextArea.prototype.onInsertImage = function(ed)
{
    ...
    if ( $("#mceImage_browser").get(0) ) {
      console.log("Custom image browser found using it");
...
{
  console.log("MCETextArea attachWith: "+name);
  if (name) {
  ...
  mce_area.tinymce({
    ...
    script_url : SITE_ROOT + 'lib/js/tiny_mce/tiny_mce.js',
  ...
  ///
    ...
    setup : function(ed) {
      //register mceImage command
      ed.addCommand('mceImage', function(ui, v) {
         return this.onInsertImage(ed);
      }, instance);
    }
