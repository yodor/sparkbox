...
MenuBarComponent.prototype.attachWith = function(name)
{
  ...
  $(this.cls).data("menu_instance", this);
  
  $(this.cls).first().find(" .MenuBarItemRenderer").each(function(index) {
	
        var bar = $(this);
	var handle = $(this).children(".handle").first();
	handle.html("&#9660;");
        ...
        if (isMob()) {
            handle.on("click", function(event){
                ...
                if (handle.data("opened")) {
                    
                    handle.data("opened", false);
                    handle.html("&#9660;");
                    
                    
                    instance.menuLeave(bar.get(0));
                    return false;
                }
                else {
  ...
  $(this.cls).first().find(" .SubmenuItemRenderer").each(function(index) {
	...
	$(this).on("touchcancel", function(event){
	  ...
	  $(this).data("touchcancel", true);
	  
	});
	
	$(this).on("touchend", function(event){
	  
		if ( $(this).data("touchmove") || $(this).data("touchcancel")) {
		  
			$(this).data("touchmove", false);
			$(this).data("touchcancel", false);
		}
		else {
