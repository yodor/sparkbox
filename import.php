...
class ImportIvona extends CSVTemplateLoader
{
    ...
    public function processDataRow($row)
    {
        ...
        $prod_row = array("product_code" => $row[ImportIvona::PRODUCT_CODE], "visible" => $row[ImportIvona::VISIBLE], "product_name" => $row[ImportIvona::PRODUCT_NAME], "product_summary" => $row[ImportIvona::PRODUCT_SUMMARY], "product_description" => $row[ImportIvona::PRODUCT_DESCRIPTION], "sell_price" => $row[ImportIvona::PRODUCT_SELL_PRICE], "buy_price" => $row[ImportIvona::PRODUCT_BUY_PRICE], "weight" => $row[ImportIvona::PRODUCT_WEIGHT], "stock_amount" => $row[ImportIvona::PRODUCT_STOCK_AMOUNT]);
            ...
            for ($a = 0; $a < count($category_branch); $a++) {
                ...
                if ($a < 1) {
                    ...
                    if ($num < 1) {
                        ...
                        $catID = $this->pc->insert($cat_row, $this->db);
