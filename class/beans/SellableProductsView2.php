...
(SELECT group_concat(pcp1.pclrpID ORDER BY pcp1.position SEPARATOR '|') FROM product_color_photos pcp1 WHERE pcp1.pclrID = si.pclrID ) as photos, 
...
(SELECT group_concat( (length(pc1.color_photo)>0) SEPARATOR '|') FROM product_colors pc1 WHERE pc1.prodID = si.prodID ) as other_color_chips,
...
si.* 
FROM sellable_products si GROUP BY si.pclrID */

    public function __construct() 
    {
	  parent::__construct("sellable_products_view2");
	  $this->prkey = "piID";
    }
